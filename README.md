...
- **Line 4** — the 7-day window, shown as its own bar only when it's ≥50% or busier than the 5-hour window. A magenta `f` at the 50% cell marks the [Fable 5 weekly cap](https://support.claude.com/en/articles/15424964-claude-fable-5-on-your-plan): on Max plans and on premium seats (Team and seat-based Enterprise), Fable is included up to 50% of the weekly limit, and past that it draws pay-as-you-go usage credits.

  It's a **landmark, not a cutover.** The bar tracks *all-model* usage while the cap applies only to *Fable* usage, so the cell means "if this week were all Fable, credits start here" — cross it on Sonnet and you owe nothing. 50% is a fixed constant because the payload carries no per-model breakdown, and for the same reason it can't tell what plan you're on: where Fable is pay-as-you-go from the start there's no included-Fable boundary, so the cell is decoration. Dynamic cells win the cell they share: clock pip > burn projection > `f`.
